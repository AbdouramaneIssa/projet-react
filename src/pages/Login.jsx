...
import { Mail, Lock, Eye, EyeOff, Sparkles, ArrowRight, Chrome, User } from 'lucide-react';
import { useHistory } from 'react-router-dom';

const Login = ({ setPage, showNotification }) => {
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      showNotification('Connexion réussie ! Bienvenue sur MealBloom ✨', 'success');
      setPage('dashboard');
    } catch (error) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      showNotification('Connexion Google réussie ! Bienvenue ✨', 'success');
      setPage('dashboard');
    } catch (error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-400 via-secondary-500 to-accent-400 p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8 animate-fade-in-down">
          ...
          <h1 className="text-4xl font-bold text-white mb-2">MealBloom</h1>
          <p className="text-white/80 text-lg">Planifiez vos repas avec style</p>
        </div>
